 ...
 * 核心思想：
 * 将数组分为已排序和未排序两部分，
 * 每次从未排序部分选择最小的元素，将找到的最小元素与未排序区间的第一个元素交换。
 *
 * 算法步骤：
 * 1. 在未排序序列中找到最小（大）元素
 ...
 * 选择排序算法实现
 * @param {Array} arr - 待排序数组
 * @returns {Array} 排序后的数组（原数组会被修改）
 * @time O(n²) - 无论输入情况如何，时间复杂度固定为O(n²)
...
function selectionSort(arr) {
  ...
  // 外层循环控制已排序区间的边界
  for (let i = 0; i < n - 1; i++) {
    let minIndex = i; // 假设未排序区间的第一个元素是最小值

    // 内层循环寻找未排序区间中的最小元素
    for (let j = i + 1; j < n; j++) {
      //如果循环的元素小于最小元素，则更新最小元素索引
 ...
 *
 * 算法步骤：
 * 1. 将第一个元素标为已排序,从第二个元素开始，将其作为待插入元素
...
class SortingAlgorithmTester {
  constructor() {
    this.algorithms = {
      ...
      堆排序: heapSort,
      计数排序: (arr) =>
        countingSort(arr.filter((n) => n >= 0 && Number.isInteger(n))),
      桶排序: bucketSort,
      基数排序: (arr) =>
        radixSort(arr.filter((n) => n >= 0 && Number.isInteger(n))),
      Tim排序: timSort,
      内省排序: introSort,
    };
  }
  ...
  performanceBenchmark(sizes = [100, 1000, 5000, 10000]) {
    ...
    for (const size of sizes) {
      ...
      for (let i = 0; i < size * 0.1; i++) {
        ...
        [nearSortedData[idx1], nearSortedData[idx2]] = [
          nearSortedData[idx2],
          nearSortedData[idx1],
        ];
      }
      ...
      for (const [caseName, data] of Object.entries(testCases)) {
        ...
        results.slice(0, 5).forEach((result) => {
          ...
          console.log(
            `${status} ${result.algorithm}: ${result.executionTime}ms`
          );
        });
      }
    }
